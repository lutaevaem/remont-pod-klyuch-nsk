...
function imageBlock(src, label) {
  if (src) {
    return `<div class="project-photo has-image"><img src="${escapeHtml(src)}" alt="${escapeHtml(label)}"><span>${escapeHtml(label)}</span></div>`;
  }
  ...
  projectContainer.innerHTML = projects.map((project) => {
    ...
    return `
      <article class="project-card project-item" data-category="${escapeHtml(categoryToFilter(project.category))}">
        ${preview ? `<div class="project-photo has-image"><img src="${escapeHtml(preview)}" alt="${escapeHtml(project.title)}"></div>` : `<div class="project-photo">${escapeHtml(categoryLabel(project.category))}</div>`}
        <h3>${escapeHtml(project.title)}</h3>
...
async function loadProjects() {
  if (!config || !window.supabase || !projectContainer || !featuredContainer) return;
