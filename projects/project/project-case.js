document.addEventListener('DOMContentLoaded', () => {
    ...
    const sections = [
      {
        title: 'Главные фото',
        cards: [
          imageCard(images.cover, 'Обложка проекта', 'Главное фото проекта.', true),
          imageCard(images.after, 'Результат', 'Финальный вид объекта.'),
          imageCard(images.before, 'Исходная точка', 'С чего начинался проект.'),
        ],
      },
      {
        title: 'Процесс и концепция',
        cards: [
          imageCard(images.process, 'Процесс работ', 'Рабочий этап проекта.', true),
          imageCard(images.concept, 'Концепция / дизайн', 'Визуальная или проектная часть.'),
        ],
      },
      {
        title: 'Дополнительные фото',
        cards: images.extra.map((url, index) => imageCard(url, `Фото ${index + 1}`, 'Дополнительное фото проекта.', index % 5 === 0)),
      },
    ];
  ...
  function renderProject(project) {
    ...
    root.innerHTML = `
      <section class="hero section-dark case-hero">
        <div class="hero-bg"></div>
        <div class="container case-hero-grid">
          <div class="case-hero-copy">
            <p class="eyebrow">Кейс · ${escapeHtml(category)}</p>
            <h1>${escapeHtml(project.title)}</h1>
            <p class="hero-lead">${escapeHtml(project.client_task || project.result || 'Подробный кейс проекта: исходная ситуация, задача, выполненные работы, результат и фотографии.')}</p>
            ${project.timeline ? `<p class="hero-note">Сроки: ${escapeHtml(project.timeline)}</p>` : ''}
            <div class="hero-actions"><a class="btn btn-primary" href="/contacts/">Обсудить похожий проект</a><a class="btn btn-secondary" href="/projects/">Вернуться к проектам</a></div>
          </div>
          ${cover ? `<button class="case-hero-visual" type="button" data-lightbox-title="${escapeHtml(project.title)}" data-lightbox-caption="Главное фото проекта."><img src="${escapeHtml(cover)}" alt="${escapeHtml(project.title)}"><span>Открыть фото</span></button>` : ''}
        </div>
      </section>

      <section class="section soft case-summary">
        <div class="container case-summary-grid">
          <article><span>Тип</span><b>${escapeHtml(category)}</b></article>
          <article><span>Площадь</span><b>${escapeHtml(project.area || 'Уточняется')}</b></article>
          <article><span>Локация</span><b>${escapeHtml(project.location || 'Новосибирск и область')}</b></article>
          <article><span>Формат</span><b>Под ключ</b></article>
        </div>
      </section>

      <section class="section">
        <div class="container section-head wide">
          <p class="eyebrow">История проекта</p>
          <h2>Как задача превратилась в готовое пространство</h2>
          <p>Эта страница собирается из данных админки: описание, этапы, результат и фотографии можно обновлять без ручной правки кода.</p>
        </div>
        <div class="container case-story-grid">
          <article class="case-story-card"><h2>С чего начали</h2><p>${escapeHtml(project.start_point || 'Исходная ситуация проекта будет дополнена в админке.')}</p></article>
          <article class="case-story-card"><h2>Задача клиента</h2><p>${escapeHtml(project.client_task || 'Задача клиента будет дополнена в админке.')}</p></article>
          <article class="case-story-card"><h2>Что сделали</h2><p>${escapeHtml(project.scope || 'Состав работ будет дополнен в админке.')}</p></article>
          <article class="case-story-card case-story-card--dark"><h2>Результат</h2><p>${escapeHtml(project.result || 'Результат проекта будет дополнен в админке.')}</p></article>
        </div>
      </section>

      <section class="section section-dark case-gallery-block">
        <div class="container section-head wide"><p class="eyebrow">Галерея проекта</p><h2>Фотографии проекта</h2><p>Фотографии открываются крупно. Новые изображения можно добавить в админке проекта.</p></div>
        <div class="container case-gallery-wrap">${renderGallery(images) || '<p class="hero-note">Фото пока не загружены. Добавьте изображения в админке проекта.</p>'}</div>
      </section>
  ...
  async function loadProject() {
    const slug = getSlug();
    if (!slug) return renderError('В адресе не указан slug проекта. Откройте проект из раздела “Проекты”.');
